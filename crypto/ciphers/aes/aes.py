...
class aes (Cipher):
	...
	num_rounds = {128:9, 192:11, 256:13, 240: 13}
	...
	def get_block(self):
		...
		## set the block to self.block
		##self.block = section
		self.gotBlock = True
		return section

	def aesEncrypt(self, pt, key):
		block = copy(pt)
		exkey = expandKey(key)
		aesMain(block, exKey)
	...
	def encrypt_block(self, block, key):
		...
		for i in range(self.num_rounds[self.key_size]-1):
			...
			self.add_round_key(state, roundKey)
			# end aes Round
		self.roundKey = self.create_round_key(exKey, self.num_rounds[self.key_size])
		self.sub_bytes(state)
		self.shift_rows(state)
		self.add_round_key(state, roundKey)
		# end aes Main
